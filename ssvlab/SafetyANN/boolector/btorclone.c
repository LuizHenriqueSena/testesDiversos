           ...
           BtorNodeMap * exp_map, BtorAIGMap * aig_map)
{
  ...
  if (BTOR_IS_PARAM_NODE (exp))
    {
      BTOR_PUSH_STACK_IF (((BtorParamNode *) exp)->lambda_exp, mm, *nodes, 
                          (BtorNode **) &((BtorParamNode *) res)->lambda_exp);
      BTOR_PUSH_STACK_IF (((BtorParamNode *) exp)->assigned_exp, mm, *nodes,
  ...
  if (BTOR_IS_LAMBDA_NODE (exp))
    {
      if (((BtorLambdaNode *) exp)->synth_apps)
        {
          BTOR_PUSH_STACK (
              mm, *sapps, &((BtorLambdaNode *) res)->synth_apps);
          BTOR_PUSH_STACK (
              mm, *sapps, &((BtorLambdaNode *) exp)->synth_apps);
        }
...
static void
data_as_node_ptr (BtorMemMgr *mm, const void * map, const void * data_ptr, 
...
static void
data_as_str_ptr (BtorMemMgr *mm, const void * str_table, const void * data_ptr, 
                 BtorPtrHashData * data)
{
  assert (mm);
  assert (str_table);
  ...
  data->asStr = (char *) btor_find_in_ptr_hash_table (
                             (BtorPtrHashTable *) str_table, str)->key;
}

static void
data_as_int_ptr (BtorMemMgr *mm, const void * map, const void * data_ptr, 
                 BtorPtrHashData * data)
{
  assert (mm);
  assert (map);
...
static void
data_as_bv_ptr (BtorMemMgr *mm, const void * map, const void * data_ptr, 
                BtorPtrHashData * data)
{
  assert (mm);
  assert (map);
...
static void
data_as_bv_htable_ptr (BtorMemMgr *mm, const void * map, const void * data_ptr, 
                       BtorPtrHashData * data)
{
  assert (mm);
  assert (map);
  ...
  data->asPtr = btor_clone_ptr_hash_table (mm, table, copy_bv_tuple,
                                           data_as_bv_ptr, map, map);
}

static void
data_as_htable_ptr (BtorMemMgr *mm, const void * map, const void * data_ptr, 
                    BtorPtrHashData * data)
{
  assert (mm);
  assert (map);
...
#define CLONE_PTR_HASH_TABLE(table) \
  do { \
    clone->table = btor_clone_ptr_hash_table (mm, btor->table, \
                                              mapped_node, 0, emap, 0); \
                ...
                int exp_layer_only)
{
  ...
  clone->symbols = btor_clone_ptr_hash_table (
                       mm, btor->symbols, clone_str_key, 0, 0, 0);
#ifndef NDEBUG
  int str_bytes = 0;
  ...
  assert ((allocated += MEM_PTR_HASH_TABLE (btor->symbols) + str_bytes)
          == clone->mm->allocated);
#endif
  clone->node2symbol = btor_clone_ptr_hash_table (
    mm, btor->node2symbol, mapped_node, data_as_str_ptr, emap, clone->symbols);
#ifndef NDEBUG
  ...
  clone->parameterized = btor_clone_ptr_hash_table (mm, btor->parameterized, 
      mapped_node, data_as_htable_ptr, emap, emap); 
  ...
  sprintf (prefix, "[%s] %s", clone_prefix, 
