...
btor_print_stats_btor (Btor * btor)
{
  ...
  if (btor->options.dual_prop.val)
    {
      BTOR_MSG (btor->msg, 1, "%.2f seconds cloning for initial applies search", 
                btor->time.search_init_apps_cloning);
      BTOR_MSG (btor->msg, 1, "%.2f seconds SAT solving for initial applies search", 
  ...
  BTOR_MSG (btor->msg, 1, "%.2f seconds determinig failed assumptions",
            btor->time.failed);
...
merge_lambdas (Btor * btor)
{
  ...
  assert (check_unique_table_merge_unset_dbg (btor));
  delta = btor_time_stamp () - start;
  BTOR_MSG (btor->msg, 1, "merged %d lambdas in %.2f seconds", delta_lambdas, delta);
}
...
optimize_unconstrained (Btor * btor)
{
  ...
  hls = btor_new_ptr_hash_table (mm, (BtorHashPtr) btor_hash_exp_by_id,
                                 (BtorCmpPtr) btor_compare_exp_by_id);
  btor_init_substitutions (btor);
...
//              || BTOR_IS_LAMBDA_NODE (cur))
            {
              for (i = cur->arity - 1; i >= 0; i--)
                hl[i] = (btor_find_in_ptr_hash_table (
                          hls, BTOR_REAL_ADDR_NODE (cur->e[i]))) ? 1 : 0;
...
btor_timed_sat_sat (Btor * btor, int limit)
{
  ...
  delta = btor_time_stamp () - start;
  btor->time.sat += delta;
  BTOR_MSG (btor->msg, 2, "SAT solver returns %d after %.1f seconds", res, delta);
  return res;
}
                                  ...
                                  BtorNodePtrStack * top_applies)
{
  ...
  set_up_dual_and_collect (btor, clone, clone_root, exp_map, &inputs,
                           top_applies, compare_scores_qsort);
#elif DP_QSORT == DP_QSORT_ASC
  set_up_dual_and_collect (btor, clone, clone_root, exp_map, &inputs,
                           top_applies, cmp_node_id_asc);
#elif DP_QSORT == DP_QSORT_DESC
  set_up_dual_and_collect (btor, clone, clone_root, exp_map, &inputs,
                           top_applies, cmp_node_id_desc);
#else
  ...
  if (!btor->dp_cmp_inputs)
#endif
    {
      ...
      set_up_dual_and_collect (btor, clone, clone_root, exp_map, &inputs,
                               &tmp_desc, cmp_node_id_desc);
      set_up_dual_and_collect (btor, clone, clone_root, exp_map, &inputs,
                               &tmp_asc, cmp_node_id_asc);
...
compute_scores (Btor * btor)
{
  ...
  if (!btor->score_depth)
    {
      btor->score_depth = btor_new_ptr_hash_table (btor->mm,
                                    (BtorHashPtr) btor_hash_exp_by_id,
                                    (BtorCmpPtr) btor_compare_exp_by_id);
    }
      ...
      while (!BTOR_EMPTY_STACK (stack))
        {
          ...
          if (cur->aux_mark == 2
              || btor_find_in_ptr_hash_table (score, cur))
            continue;
          ...
          if (cur->aux_mark == 0)
          ...
          else
            {
              ...
              assert (!btor_find_in_ptr_hash_table (score, cur));
              btor_insert_in_ptr_hash_table (
                  score, btor_copy_exp (btor, cur))->data.asInt = min_depth + 1;
            }
        }
    }
              ...
              if (BTOR_IS_APPLY_NODE (cur) && !cur->parameterized)
                {
                  ...
                  btor_insert_in_ptr_hash_table (in,
                      btor_copy_exp (btor, cur));
                }
#else
              assert (!BTOR_IS_APPLY_NODE (cur));
...
propagate (Btor * btor, BtorNodePtrStack * prop_stack,
           BtorPtrHashTable * cleanup_table, int * assignments_changed)
{
  assert (btor);
  assert (prop_stack);
...
BTOR_CONFLICT_CHECK:
  ...
  if (!btor->searched_applies)
    {
      btor->searched_applies = btor_new_ptr_hash_table (btor->mm,
                                   (BtorHashPtr) btor_hash_exp_by_id,
                                   (BtorCmpPtr) btor_compare_exp_by_id);
    }
...
DONE:
  BTOR_RELEASE_STACK (btor->mm, prop_stack);
  btor->valid_assignments = 1;
  BTOR_ABORT_CORE (lod_limit == -1 && sat_limit == -1
                   && sat_result != BTOR_SAT && sat_result != BTOR_UNSAT,
...
br_probe (Btor * btor)
{
  ...
  if (res != BTOR_UNKNOWN)
  ...
  else if (num_ops_clone < num_ops_orig * btor->options.pbra_ops_factor.val)
    {
      ...
      res = btor_sat_aux_btor (clone, btor->options.pbra_lod_limit.val, 
          btor->options.pbra_sat_limit.val);
      btor_delete_btor (clone);
    }
  ...
  if (argc == 1)
    {
      ...
      if (btor_is_fun_exp (btor, args[0])
          || btor_is_array_exp (btor, args[0])
      ...
      for (i = 0; i < argc; i++)
        {
          ...
          /* NOTE: we do not allow functions or arrays as arguments yet */
          if (btor_is_fun_exp (btor, args[i])
              || btor_is_array_exp (btor, args[i])
...
fun_sort_from_fun (Btor * btor, BtorNode * fun)
{
  ...
  sort = btor_fun_sort (&btor->sorts_unique_table, domain.start,
...
btor_create_or_get_sort (Btor * btor, BtorNode * exp)
{
  ...
  if (BTOR_IS_FUN_NODE (exp))
    return fun_sort_from_fun (btor, exp);
#if 0
          ...
          switch (real_cur->kind)
            {
              case BTOR_SLICE_NODE:
                result =
                  btor_slice_bv (btor, e[0], real_cur->upper, real_cur->lower);
...
void
btor_array_assignment_str (Btor * btor, BtorNode * exp, char ***indices,
      ...
      for (i = 0; i < size; i++)
        {
          ...
          fprintf (file, "%d[%s] %s %s\n", id ? id : node->id, 
                   pretty_args, pretty_val, symbol ? symbol : "");
...
reset_varsubst_constraints (Btor * btor)
{
  ...
  btor_delete_ptr_hash_table (btor->varsubst_constraints);
  btor->varsubst_constraints = btor_new_ptr_hash_table (btor->mm,
                                   (BtorHashPtr) btor_hash_exp_by_id,
                                   (BtorCmpPtr) btor_compare_exp_by_id);
}
...
static BtorNode *
generate_lambda_model_from_fun_model (Btor * btor, BtorNode * exp,
...
check_model (Btor * btor, Btor * clone, BtorPtrHashTable * inputs)
{
  ...
  if (clone->valid_assignments)
    btor_reset_incremental_usage (clone);
