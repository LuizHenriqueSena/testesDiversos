...
static double powf_log_table[256] = {
...
float nnPowf( float x, float y )
{
	...
	if( m.f != 1.0f )
	{
		...
		double *tablep = (void *)powf_log_table + index;
		double r = (double) m.f;
...
nan_sqrt:
	...
	return (float) nnSqrt( 1.0 / (double) x );
