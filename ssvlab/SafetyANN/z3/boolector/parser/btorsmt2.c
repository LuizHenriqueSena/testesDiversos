...
ONE_FIXED_NUM_PARAMETRIC:
                          ...
                          l = p - 1;
                          if (!btor_parse_int32_smt2 (parser, 0, &l->num))
                            return 0;
                          l->tag = tag;
                          l->node = node;
                          parser->work.top = p;
                      ...
                      else if (tag == BTOR_EXTRACT_TAG_SMT2)
                        {
                          ...
                          if (BTOR_COUNT_STACK (parser->work) < 3
                            ...
                            goto ONE_FIXED_NUM_PARAMETRIC;
                          l = p - 1;
                          if (!btor_parse_int32_smt2 (parser, 0, &l->hi))
                            return 0;
                          firstcoo = parser->coo;
                          if (!btor_parse_int32_smt2 (parser, 0, &l->lo))
                            return 0;
                          if (l->hi < l->lo)
                            {
                        ...
                        BtorParseResult * res)
{
  ...
  if (!parser->commands.all)
    BTOR_MSG (boolector_get_btor_msg (parser->btor), 1, 
        "WARNING no commands in '%s'", parser->name);
  else
    {
      if (!parser->commands.set_logic)
        BTOR_MSG (boolector_get_btor_msg (parser->btor), 1,
