...
btormain_init_opts (BtorMainApp * app)
{
  ...
  BTORMAIN_INIT_OPT (app->opts.help, 0, "h", "help", 0, 0, 1,
            "print this message and exit");
  BTORMAIN_INIT_OPT (app->opts.copyright, 0, "c", "copyright", 0, 0, 1,
            "print copyright and exit");
  BTORMAIN_INIT_OPT (app->opts.version, 0, "V", "version", 0, 0, 1,
            "print version and exit");
  BTORMAIN_INIT_OPT (app->opts.time, 0, "t", "time", 0, 0, -1,
            "set time limit");
  BTORMAIN_INIT_OPT (app->opts.output, 1, "o", "output", 0, 0, 0, 
            "set output file for dumping");
#ifdef BTOR_USE_LINGELING
  BTORMAIN_INIT_OPT (app->opts.lingeling, 1, 0, "lingeling", 0, 0, 1,
            "force Lingeling as SAT solver");
  BTORMAIN_INIT_OPT (app->opts.lingeling_opts, 1, 0, "lingeling_opts", 
            0, 0, 0, "set lingeling option(s) '--<opt>=<val>'");
  BTORMAIN_INIT_OPT (app->opts.lingeling_nofork, 1, 0, "lingeling_nofork", 
            0, 0, 0, "do not use 'fork/clone' for Lingeling");
#endif
#ifdef BTOR_USE_PICOSAT
  BTORMAIN_INIT_OPT (app->opts.picosat, 1, 0, "picosat", 0, 0, 1,
            "force PicoSAT as SAT solver");
#endif
#ifdef BTOR_USE_MINISAT
  BTORMAIN_INIT_OPT (app->opts.minisat, 1, 0, "minisat", 0, 0, 1,
           ...
           const char * desc)
{
  ...
  if (!strcmp (lng, BTOR_OPT_REWRITE_LEVEL)
      ...
      || !strcmp (lng, BTOR_OPT_PBRA_SAT_LIMIT)
      || !strcmp (lng, BTOR_OPT_PBRA_OPS_FACTOR)
      || !strcmp (lng, BTOR_OPT_DUAL_PROP)
      || !strcmp (lng, BTOR_OPT_JUST)
      ...
      || !strcmp (lng, BTOR_OPT_VERBOSITY)
      || !strcmp (lng, BTOR_OPT_LOGLEVEL))
    {
  ...
  for (i = 1; i < argc; i++)
    {
          ...
          if (!btor_file_exists (static_app->infile_name))
                   ...
                   || has_suffix (static_app->infile_name, "7z"))
            {
              ...
              if (has_suffix (static_app->infile_name, ".gz"))
                ...
                sprintf (
                    cmd, "7z x -so %s 2>/dev/null", static_app->infile_name);
      ...
      shrt = argv[i][1] == '-' ? 0 : 1;
      j = shrt ? 1 : 2;
      disable = argv[i][j] == 'n' && argv[i][j+1] == 'o' && argv[i][j+2] == '-';
      ...
      if (argv[i][j] == '=')
      ...
      else if (i+1 < argc && argv[i+1][0] != '-')
        {
          ...
          if (tmp[0] == 0) { readval = 1; i += 1; }
        }

      if ((shrt 
           && static_app->opts.help.shrt
          ...
          || (!shrt && !strcmp (opt, static_app->opts.help.lng)))
        {
          ...
          goto DONE;
                ...
                && static_app->opts.copyright.shrt
               ...
               || (!shrt && !strcmp (opt, static_app->opts.time.lng)))
        {
          ...
          if (!readval)
            {
              btormain_error (
                  static_app, "missing argument for '%s%s'", 
                  shrt ? "-" : "--", opt);
              goto DONE;
            }
          ...
          if (static_set_alarm <= 0)
            {
              btormain_error (
                  static_app, "invalid argument for '%s%s'", 
                  shrt ? "-" : "--", opt);
              goto DONE;
            }
        }
               ...
               || (!shrt && !strcmp (opt, static_app->opts.output.lng)))
        {
          ...
          static_app->close_outfile = 1;
                ...
                && static_app->opts.lingeling.shrt
               ...
               || (!shrt && !strcmp (opt, static_app->opts.lingeling.lng)))
        {
          ...
          static_app->opts.lingeling.val = 1;
                ...
                && static_app->opts.lingeling_opts.shrt
          ...
          if (!strcmp (opt, "btor"))
            {
              ...
              boolector_set_opt (static_app->btor, 
                  BTOR_OPT_INPUT_FORMAT, BTOR_INPUT_FORMAT_BTOR);
            }
          else if (!strcmp (opt, "smt2"))
            {
              ...
              boolector_set_opt (static_app->btor, 
                  BTOR_OPT_INPUT_FORMAT, BTOR_INPUT_FORMAT_SMT2);
            }
          else if (!strcmp (opt, "smt1"))
            {
              ...
              boolector_set_opt (static_app->btor, 
                  BTOR_OPT_INPUT_FORMAT, BTOR_INPUT_FORMAT_SMT1);
            }
          else if (!strcmp (opt, "x") || !strcmp (opt, "hex"))
            {
              ...
              boolector_set_opt (static_app->btor, 
                  BTOR_OPT_OUTPUT_NUMBER_FORMAT, BTOR_OUTPUT_BASE_HEX);
            }
          else if (!strcmp (opt, "d") || !strcmp (opt, "dec"))
            {
              ...
              boolector_set_opt (static_app->btor, 
                  BTOR_OPT_OUTPUT_NUMBER_FORMAT, BTOR_OUTPUT_BASE_DEC);
            }
            ...
            {
              for (o = (char *) boolector_first_opt (static_app->btor); 
                   o; 
              ...
              if (!o)
                {
                  btormain_error (static_app, "invalid option '%s%s'",
                      shrt ? "-" : "--", opt);
                  goto DONE;
                }
              ...
              if ((shrt && oshrt && !strcmp (oshrt, "i"))
                       ...
                       || (!shrt && !strcmp (o, BTOR_OPT_INCREMENTAL_ALL)))
                {
                  if (disable || (readval && val == 0))
                    ...
                    {
                      boolector_set_opt (static_app->btor, o, 
                          BTOR_PARSE_MODE_INCREMENTAL_BUT_CONTINUE);
                      inc |= BTOR_PARSE_MODE_INCREMENTAL_BUT_CONTINUE;
                      boolector_set_opt (static_app->btor, 
                          BTOR_OPT_INCREMENTAL, inc);
                    }
                }
              else if ((!shrt 
                        && !strcmp (o, BTOR_OPT_INCREMENTAL_IN_DEPTH)))
                {
                  if (disable)
                    {
                      btormain_error (static_app, "invalid option '%sno-%s'", 
              ...
              else
                {
                  ...
                  if ((!strcmp (o, BTOR_OPT_DUAL_PROP)
                       && boolector_get_opt_val (
                              static_app->btor, BTOR_OPT_JUST))
                      || (!strcmp (o, BTOR_OPT_JUST)
                          && boolector_get_opt_val (
                                 static_app->btor, BTOR_OPT_DUAL_PROP)))
                    {
                      btormain_error (static_app, 
  ...
  if (!inc && (incid || incla || incint))
    {
      inc = 1;
      boolector_set_opt (static_app->btor, 
          BTOR_OPT_INCREMENTAL, BTOR_PARSE_MODE_BASIC_INCREMENTAL);
    }
  ...
  if (static_app->opts.lingeling.val)
    {
      ...
      if (!boolector_set_sat_solver_lingeling (static_app->btor, 
            lingeling_opts, static_app->opts.lingeling_nofork.val))
        btormain_error (static_app, 
            "invalid options to Lingeling: '%s'", lingeling_opts);
    }
#endif
#ifdef BTOR_USE_PICOSAT
...
#error "no SAT solver configured"
  ...
  if (static_verbosity)
    {
      if (inc)
        btormain_msg ("incremental mode through command line option");
      if (incid)
        btormain_msg ("incremental in-depth window of %d", incid);
      if (incla)
        btormain_msg ("incremental look-ahead window of %d", incla);
      if (incint)
        btormain_msg ("incremental interval window of %d", incint);
  ...
  if (inc && static_verbosity)
    btormain_msg ("starting incremental mode");
  ...
  if ((val = boolector_get_opt_val (static_app->btor, "input_format")))
      ...
      switch (val)
        {
          case BTOR_INPUT_FORMAT_BTOR:
            ...
            parse_result =
              boolector_parse_btor (static_app->btor, static_app->infile,
                  static_app->infile_name, &parse_error_msg, &parse_status);
            break;
          case BTOR_INPUT_FORMAT_SMT1:
            if (static_verbosity)
              btormain_msg ("SMT-LIB v1 input forced through cmd line options");
            parse_result =
              boolector_parse_smt1 (static_app->btor, static_app->infile,
                  static_app->infile_name, &parse_error_msg, &parse_status);
            break;
          case BTOR_INPUT_FORMAT_SMT2: 
            ...
            parse_result =
              boolector_parse_smt2 (static_app->btor, static_app->infile,
                  static_app->infile_name, &parse_error_msg, &parse_status);
            break;
        }
    }
    ...
    parse_result =
      boolector_parse (static_app->btor, static_app->infile, 
          static_app->infile_name, &parse_error_msg, &parse_status);
      ...
      switch (dump)
        {
          case BTOR_OUTPUT_FORMAT_BTOR:
            if (static_verbosity)
              btormain_msg ("dumping BTOR expressions");
...
DONE:
  if (static_app->done) res = BTOR_SUCC_EXIT;
  else if (static_app->err) res = BTOR_ERR_EXIT;
