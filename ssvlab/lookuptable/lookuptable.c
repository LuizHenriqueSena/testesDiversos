...
float tanhFunction(float u){
	float retorno;
	retorno = ((powf(2.718281,u) - powf(2.718281,(u*(-1)))) / (powf(2.718281,u) + powf(2.718281,(u*(-1)))));
	return retorno;
}
...
void printLookUpTableForCVectorTanh(int number, int decimalPlaces) {
	int sizetable = 0;
	int shift = 0;
	int decimalNumber = (int)pow(10, decimalPlaces);
	shift = number/2;
	...
	int i = 1;
	printf("float lookup[%d] = {", sizetable);
	printf("%.6f ,", tanhFunction(-shift));
	for(i=1;i<sizetable;i++) {
			printf("%.6f ,", tanhFunction((((float)i/(float)decimalNumber)-(float)shift)));
			if(i==sizetable-1) {
				printf("};\n");
			}
			else if(i%10==9){
