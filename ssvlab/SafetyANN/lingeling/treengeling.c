...
static int splitsuccessful = 1, branches = -1;
...
static int64_t sumclims, inclims, declims, forcedclims;
...
static void lockparleafs () { LOCK (parleafs); }
...
static void lockleafs () { LOCK (leafs); }
...
static void unlockparleafs () { UNLOCK (parleafs); }
...
static void unlockleafs () { UNLOCK (leafs); }
...
static void report () {
  ...
  msg ("%c%d %lld%c %.1f sec, "
    "%.0f MB, %d nodes +%d -%d, vars [%d..%d..%d]",
    (isfull ? '[' : '('), round, (LL) ids, (isfull ? ']' : ')'), t, m,
    numnodes, added, deleted, minvars, actvars, maxvars);
}
...
static void stats () {
  ...
  msg ("%.0f avg clim, %d incs, %d decs, %d forced", 
      avg (sumclims, inclims + declims + 1), inclims, declims, forcedclims);
  msg ("");
...
static int finish () {
  ...
  if (i < numnodes) {
    ...
    res = parallel.res;
    unlockdone ();
    vrb ("no node reslut thus sat parallel result %d", res);
