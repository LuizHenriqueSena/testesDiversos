...
add_to_bv_model (Btor * btor, BtorNode * exp, BitVector * assignment)
{
  ...
  if (btor_find_in_ptr_hash_table (btor->bv_model, exp))
    return;
...
static void
add_to_fun_model (Btor * btor, BtorNode * exp,
                    BitVectorTuple * t, BitVector * value)
{
  assert (btor);
  assert (exp);
  assert (BTOR_IS_REGULAR_NODE (exp));
  ...
  if (b)
  ...
  else
    {
      ...
      btor_insert_in_ptr_hash_table (btor->fun_model,
  ...
  while (!BTOR_EMPTY_STACK (work_stack))
    {
      ...
      if (btor_find_in_ptr_hash_table (assignments, real_cur))
        goto PUSH_CACHED;
          ...
          if (!real_cur->parameterized)
            {
              ...
              btor_insert_in_ptr_hash_table (assignments,
                  real_cur)->data.asPtr = btor_copy_bv (btor, result);
            }
          else
            real_cur->eval_mark = 0;
...
static void
compute_lambda_model (Btor * btor, BtorNode * exp,
                      BtorPtrHashTable * assignments)
{
  assert (btor);
  assert (exp);
...
btor_has_bv_model (Btor * btor, BtorNode * exp)
{
  ...
  return btor_find_in_ptr_hash_table (btor->bv_model,
              BTOR_REAL_ADDR_NODE (exp)) != 0;
}
...
void
btor_get_fun_model_str (Btor * btor, BtorNode * exp,
